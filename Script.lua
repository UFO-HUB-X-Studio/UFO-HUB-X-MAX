--========================================================
-- UI MAX Script.lua (Full Version)
-- UFO HUB X — Boot Loader (Key → Download → Main UI)
--========================================================
-- รองรับ Delta / syn / KRNL / Script-Ware / Fluxus / Solara ฯลฯ + loadstring(HttpGet)
-- Patch Key/Download ให้ยิงสัญญาณ, Watchers หลายชั้น, Retry/Backoff, Force Main fallback
-- FORCE_KEY_UI = true → Key UI โชว์เสมอ
-- Hotkey: RightAlt → ลบคีย์ + รีโหลด
-- External Key (Encrypted) → UFO-HUB-X-Studio/UFO-HUB-X-key1
...
--========================================================
-- External key save + import (ENCRYPTED)
--========================================================
...
local function readKeyExternal()
    ...
    -- legacy not handled here for brevity
    return nil
end
...
local FORCE_KEY_UI = true -- บังคับให้ Key UI ขึ้นเสมอ
...
-- Force Key First (override) - บังคับให้ UI ขึ้นเสมอ
FORCE_KEY_UI = true

-- อ่าน state ปกติ (แต่เราเลือกบังคับ UI)
local cur   = readState()
local valid = isKeyStillValid(cur)

-- (พยายามนำเข้าจาก external หากไม่มี state และยังต้องการ - โค้ดนี้ไม่บังคับใช้
--  แต่เก็บไว้เป็นตัวเลือก หากต้องการให้ import อัตโนมัติ ให้เปิดส่วนนี้)
-- do
--     if not valid then
