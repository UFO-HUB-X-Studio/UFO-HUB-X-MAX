...
--========================================================
-- Auto Clear Old State + Auto Import (ไม่ต้องทำอะไรเอง)
--========================================================
task.spawn(function()
    if isfile and isfile("UFOHubX/key_state.json") and delfile then
        pcall(delfile, "UFOHubX/key_state.json")
        log("Auto-cleared old key_state.json")
    end
    local ext = readKeyExternal()
    if ext and ext.key and #tostring(ext.key) > 0 then
        local okToUse = false
        ...
        if okToUse and _G and type(_G.UFO_SaveKeyState)=="function" then
            _G.UFO_SaveKeyState(ext.key, ext.expires_at, ext.permanent)
            log("Auto-imported key from encrypted file → "..tostring(ext.key))
        end
    end
end)
