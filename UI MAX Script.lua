...
-- Boot ด้วย pcall (กันพัง + แจ้งเตือนถ้ามี error)
local ok_boot, err_boot = pcall(_ufo_boot)
if not ok_boot then
    warn("[UFO-HUB-X] UI init error: ", err_boot)
    pcall(function()
        local SG = Instance.new("ScreenGui")
        SG.Name = "UFO_Fallback"
        SG.ResetOnSpawn = false
        local parent = (gethui and gethui()) or CG
        SG.Parent = parent
        local L = Instance.new("TextLabel", SG)
        L.Size = UDim2.fromOffset(520, 44)
        ...
        L.TextSize = 16
        L.TextWrapped = true
        L.Text = "UFO HUB X UI failed to load: "..tostring(err_boot)
    end)
end
```0
